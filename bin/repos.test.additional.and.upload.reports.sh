...
# Clean has to be executed, as otherwise the code coverage report will be missing for `repos.test.additional.and.upload.reports.sh`.
# Furthermore, one cannot execute both mvn commands as one, as otherwise the `repos.test.additional.and.upload.reports.sh` will not work as well.
# JavaDocs are primarily build here and uploaded to `repos.test.additional.and.upload.reports.sh` and not at other places,
