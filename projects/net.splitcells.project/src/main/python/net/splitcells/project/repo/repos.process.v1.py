...
if __name__ == '__main__':
	if environ.get('repo_process_v2') == '1':
		print('Using ' + str(['repos.process'] + sys.argv[1:]))
		exit(subprocess.call(['repos.process'] + sys.argv[1:]))
	...
	# The following arguments are only present, so that the tool has the same API as v2.
	parser.add_argument('--dry-run', dest='dryRun', required=False, default=False, help="If true, commands are only prepared and no commands are executed.")
	parser.add_argument('--verbose', dest='verbose', required=False, default=False, help="If set to true, the output is verbose.")
	parser.add_argument('--process-in-parallel', dest='processInParallel', required=False, default='false')
