...
public class HasMinimalDistanceOfCallRunner implements FunctionCallRunner {
    ...
    private static final FunctionCallRunnerParser<Args> PARSER = functionCallRunnerParser(MINIMAL_DISTANCE_NAME, 1
            , fcr -> {
                val args = new Args();
                fcr.requireArgumentCount(2);
                args.distanceAttribute = fcr.parseAttributeArgument(Integer.class, 0, "distanceAttribute");
                args.minimalDistance = fcr.parseArgument(Integer.class, 1, "minimalAttribute");
