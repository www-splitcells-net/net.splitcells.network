...
public class Dem {
    ...
    /**
     * <p>Executes {@code cellClass} with some disabled {@link Option}, in order to get the {@link Configuration},
     * that is generated by {@code cellClass}.
     * The {@link Configuration} is serialized and compared to the {@code expectedCommonMarkSerialization}.
     * This is used, in order to check, if there are unexpected changes in the {@link Configuration} during the development.
     * Some {@link Option}s like the web server are disabled via {@link StartServicesAutomatically},
     * as these cause side effects on the host and may even make executing {@code cellClass} impossible.</p>
     * <p>There is an additional logging regarding the actual configuration,
     * so it is easily copy and pastable from the test output.</p>
     *
     * @param cellClass
